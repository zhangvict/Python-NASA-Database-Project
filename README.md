# NASA Database Python Project - Explore Close Approaches of Near-Earth Objects (Udacity Intermediate Python)
The following is a summary of the functionality of the project, filtered from the starter code assingment file in the original forked repository: https://github.com/udacity/cd0010-advanced-python-techniques-project-starter/blob/master/README.md.

# Attributions and Contributions:
The project involves a command-line engine for parsing user input. This engine was completely implemented in the starter code and the author made no contributions.
Unit tests were also completely provided by the starter code.
Aside from the above, all other remaining parts were contributed to by the author, from the skeleton module files provided by the starter code. These are modules for 1) designing classes to store the database information and connect the interrelated databases, 2) extracting raw data into the custom datatypes databases for manipulation 3) writing methods for filtering information in custom classes and 4) transforming the data back from the custom datatypes into raw output.
The purpose of this project was to demonstrate low-level knowledge of data structures and object-oriented programming in organizing manipulating database data. Higher level principles of documenting code, writing and combining the modules themselves with scalability in mind, are currently being pursued in the second project of the class.
## Overview
The project is an implementation of a custom command-line tool that is adapted to performing inspections and queries on those particular datasets.
The first dataset: neos.csv describes of objects in space, called ‘Near Earth Object’ or NEO’s, by various attributes such as id number, name, or size and more.
The second dataset: cad.json describes events where Near Earth Objects have passed closed to earth; such events are called ‘Close Approaches’. The events are described by properties such as id number of the Near Earth Object passing, velocity, distance, and more.
The custom query allows us to filer Close Approaches by any combination of the following properties:
...
## Project Interface
This project is run by the `main.py` script.
At a command line, running `python3 main.py --help` gives instruction for invoking the script.
```python
...
```
There are three subcommands: `inspect`, `query`, and `interactive`. Each is described below.
### `inspect`
...
### `interactive`
This subcommand first loads the database and then starts a command loop so the user can run `inspect` and `query` subcommands on the database without having to wait to reload the data each time you want to run a new command, which saves an extraordinary amount of time.
Example Session:
```
...
#### Testing
Unit tests are supplied to check the functionality of the completed project. They are run as follows:
```
$ python3 -m unittest
.........................................................................
----------------------------------------------------------------------
